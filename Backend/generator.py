...
def pdftojson(pdf):
    ...
    promptquiz = """I am having trouble understanding Datapaths. Here are the lecture information associated with it. 
    Can you make a multiple choice quiz with 5 questions, and give me the answer key at the end? Return output in json format"""

    promptsum = """I am having trouble understanding Datapaths. Here is the lecture information associated with it.
    Can you summarize that for me. Return output strictly in JSON format: {Summary: summary}. """
...
def imgtojson(img):
    ...
    genai.configure(api_key=os.environ["GOOGLE_API_KEY"])
    model = genai.GenerativeModel('gemini-1.5-pro-latest')
    prompt = """I am having trouble understanding Datapaths. Here is an image related to this. Can you su
    mmarize that for me. Return output strictly in JSON format: {Summary: summary}. """
    image = PIL.Image.open(img)
    input=[prompt, image]
    response = model.generate_content(inputsum)
    with open('./outpic.json', 'w') as f:
        f.write(response.text)
    with open('./outpic.json', 'r+') as f:
        lines = f.readlines()
        f.seek(0)
        f.truncate()
        f.writelines(lines[1:-1])
...
As a professor focusing on that field, can you summarize that for me, organizing it by key subjects? Return output in json format."""
